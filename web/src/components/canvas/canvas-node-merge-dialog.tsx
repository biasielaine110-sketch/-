...
export type CanvasImageMergeParams = {
    ...
    rows: number;
    columns: number;
    pieces: Array<{ row: number; column: number; dataUrl: string; nodeId?: string }>;
...
export function CanvasNodeMergeDialog({
    ...
    useEffect(() => {
        if (!open) return;
        const nextOrientation: MergeOrientation = imageCount === 3 ? "horizontal" : "grid";
        const layout = resolveMergeLayout(Math.max(2, imageCount), nextOrientation);
        const sorted = sortCandidates(candidates, nextOrientation);
        ...
        const nextSlots = buildInitialSlots(sorted, layout.rows, layout.columns, nextOrientation);
    ...
    const applyOrientation = (next: MergeOrientation) => {
        const layout = resolveMergeLayout(Math.max(filledCount || imageCount, 2), next);
        ...
        <Modal title={null} open={open} onCancel={onClose} footer={null} width={920} centered destroyOnHidden transitionName="" maskTransitionName="">
            <div className="space-y-5" data-canvas-no-zoom data-canvas-shortcuts-ignore>
                <div>
                    ...
                    <p className="mt-1 text-sm opacity-60">{t("canvas.editors.mergeVisualDescription", { count: imageCount })}</p>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex max-h-28 flex-wrap gap-2 overflow-y-auto rounded-lg border border-dashed p-2">
                                {poolKeys.length ? (
                                    ...
                                    <span className="px-1 py-2 text-xs opacity-45">{t("canvas.editors.mergePoolEmpty")}</span>
                                )}
                            </div>
                        </div>
                    </div>
